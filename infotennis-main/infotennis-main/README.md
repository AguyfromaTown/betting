...
## GitHub-compatible ATP/WTA collector

The betting-bot integration uses `infotennis.scrapers.public_data` as its
primary source catalog. It collects freely available, daily-updated ATP, WTA,
Challenger, qualifying and ongoing match CSVs from TennisMyLife without MySQL,
Selenium or browser automation. The original ATP/Infosys routines remain
optional because those endpoints may return HTTP 403 to automated runners.
The betting bot derives rankings-at-match, player metadata, recent form,
head-to-head, serve/return statistics, overall Elo and surface Elo locally.
