...
class TennisBotTests(unittest.TestCase):
    ...
    def test_daily_workflow_runs_three_incremental_scheduled_scans(self):
        ...
        self.assertIn("default: '1.50'", workflow)
...
```"""
    ...
    def test_dashboard_uses_automated_data_sources_and_wl_results(self):
        ...
        self.assertIn('id="segment-body"', dashboard)
        self.assertIn("function renderPerformanceSegments(rows)", dashboard)
        self.assertIn("function competitiveLevel(tour)", dashboard)
        self.assertIn('["Tour", row =>', dashboard)
        self.assertIn('["Surface", row =>', dashboard)
        self.assertIn('["Level", row =>', dashboard)
        self.assertIn('const KILL_SWITCH_URL = RAW_ROOT + "kill-switch.json"', dashboard)
        self.assertIn('id="runtime-control"', dashboard)
        self.assertIn('id="model-version"', dashboard)
        self.assertIn('id="policy-version"', dashboard)
        self.assertIn("function renderRuntimeSafety(audit, policies)", dashboard)
        self.assertIn("EMERGENCY STOP ACTIVE", dashboard)
        self.assertIn("invalid_kill_switch_configuration", dashboard)
        self.assertIn('const SOURCE_HEALTH_URL = RAW_ROOT + "source-health.json"', dashboard)
        self.assertIn('id="source-health-body"', dashboard)
        self.assertIn('id="quota-health-body"', dashboard)
